...
namespace TouchFramework.ControlHandlers
{
    ...
    /// </summary>
    public class PhotoHandler : ElementHandler
    {
        ...
        {
            Photo s = Source as Photo;
            Canvas parent = s.Parent as Canvas;
            //if (p.X < 20 && p.Y < 20)
            if (relative.X < 20 && relative.Y < 20)
            {
                //s.framework.Stop();
                //s.framework.UnregisterElement(s.Id);
                //parent.Children.Remove(s);
                Canvas.SetTop(s, (int)3000);
                Canvas.SetZIndex(s, (int)0);
